...
Yanfly.SEL.version = 1.07;

//=============================================================================
 /*:
 * @plugindesc v1.07 Enable specified maps to memorize event locations and tile changes.
 ...
 *
 * This plugin now also saves tile changes made during gameplay. When you
 * change tiles via events (e.g., "Change Tile" command) and then save, the
 * tile states will be preserved upon loading, even after closing and restarting
 * the game.
 *
 * ============================================================================
 * Notetags
 ...
 * - Finished plugin!
 *
 * Version 1.07 (Modified):
 * - Added tile data saving/loading: tile changes are now preserved on save/load.
 * - Added forced restoration of tile data after loading a save file, ensuring
 *   tile changes survive even after closing and restarting the game.
 */
//=============================================================================
...
Game_System.prototype.initSavedEventLocations = function() {
  ...
  this._savedTileData = {};  // store tile data per map
};
...
// --- Tile data methods ---
Game_System.prototype.savedTileData = function() {
  if (this._savedTileData === undefined) {
    this._savedTileData = {};
  }
  return this._savedTileData;
};

Game_System.prototype.getSavedTileData = function(mapId) {
  return this.savedTileData()[mapId] || null;
};

Game_System.prototype.setSavedTileData = function(mapId, data) {
  if (data && data.length > 0) {
    this.savedTileData()[mapId] = data.slice(); // store a copy
  } else {
    delete this.savedTileData()[mapId];
  }
};
...
Game_Map.prototype.setup = function(mapId) {
  // Save tile data of the current map before switching (if different)
  if (this._mapId && this._mapId !== 0 && this._mapId !== mapId) {
    if (this._map && this._map.data) {
      $gameSystem.setSavedTileData(this._mapId, this._map.data);
    }
  }

  // Call original setup (this sets this._map = $dataMap)
  Yanfly.SEL.Game_Map_setup.call(this, mapId);

  // Replace this._map with an independent copy (so we don't modify $dataMap)
  var originalMap = $dataMap;
  var newMap = Object.assign({}, originalMap);
  newMap.data = originalMap.data.slice(); // deep copy of tile data
  this._map = newMap;

  // Restore saved tile data if any (from Game_System)
  var saved = $gameSystem.getSavedTileData(mapId);
  if (saved) {
    this._map.data = saved.slice(); // use the saved data
  }
...
// Helper to save current tile data to Game_System (called on save)
Game_Map.prototype.saveTileDataToSystem = function() {
  if (this._map && this._map.data) {
    $gameSystem.setSavedTileData(this.mapId(), this._map.data);
  }
};
...
//=============================================================================
// DataManager - Save/Load Hooks for Tile Data
//=============================================================================

// Save: store current map tile data into Game_System before creating save contents
Yanfly.SEL.DataManager_makeSaveContents = DataManager.makeSaveContents;
DataManager.makeSaveContents = function() {
  if ($gameMap) {
    $gameMap.saveTileDataToSystem();
  }
  return Yanfly.SEL.DataManager_makeSaveContents.call(this);
};

// Load: after extracting save contents, force restore tile data from Game_System
Yanfly.SEL.DataManager_extractSaveContents = DataManager.extractSaveContents;
DataManager.extractSaveContents = function(contents) {
  Yanfly.SEL.DataManager_extractSaveContents.call(this, contents);
  // Now restore tile data for the current map (if any)
  if ($gameMap && $gameSystem) {
    var mapId = $gameMap.mapId();
    var saved = $gameSystem.getSavedTileData(mapId);
    if (saved && $gameMap._map && $gameMap._map.data) {
      // Overwrite current map's tile data with saved data
      $gameMap._map.data = saved.slice();
    }
  }
};
