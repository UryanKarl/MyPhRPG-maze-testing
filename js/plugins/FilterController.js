 ...
 * @param Use Decimal in Variables
 * @text 允许变量使用小数点
 * @desc 如果为true，则四舍五入到小数点四位以下，
 * 变量可以使用小数点。输入值:true/false
 * @default true
 * 
 * @param displacementImage
 * @text 置换图像名
 * @desc 置换的图像的名称。
 * 请将图像放入img/pictures/中。
 * @default DisplacementMap
 * @dir img/pictures
 * @type file
 * 
 ...
 * @text 在选项菜单中显示
 * @parent enabledAll-Settings
 * @desc 可以从游戏内的选项菜单中设置「滤镜效果」的ON/OFF
 * @type boolean
 * @default true
 * 
 * @param enabledAll-Text
 * @text 选项菜单中的文本
 * @parent enabledAll-Settings
 * @desc 游戏内选项中的滤镜效果ON/OFF的文本
 * @default 滤镜效果
 * 
 * @param enabledAll-DefaultValue
 * @text 滤镜效果ON/OFF的初始值
 * @parent enabledAll-Settings
 * @desc 滤镜效果ON/OFF的初始值；OFF→不应用所有滤镜
 * @type boolean
 * @default true
 * 
 * @help
 ...
 *  （参数之间用半角空格隔开）
 * 
 * 　createFilter [id] [效果名称] [效果目标] ([角色ID])
 *    制作画面效果。
 *    - id: 效果的可分辨名称。不是数字也可以。
 *    - 效果名称: 效果类型。
 *    - 效果目标: 0: 画面
 *                   1: 全屏(包括信息窗口)
 *                   2: 地图（所有角色+地形图块）、21:（上述+parallax）
 *                   3: 地形图块、31: 图块+parallax、32: 仅parallax
 *                   4: 所有角色
 *                   4000+x: 特定角色(x-> -1: 自身角色, 
 *                           0: 此事件, 1以上:对应事件)
 *                   5: 所有图片
 *                   5000+x: 特定图片 (x-> 图片编号)
 *      、、 、
 * 
 *    - (角色ID): 如果指定，则将效果附加到相应的角色，
 *                 如果离开地图/战斗，效果将消失。
 *        地图： -1: 自身角色, 0: 此事件, 1以上: 对应事件
 *        战斗： -1以下: 敌人, 0: 此敌人, 1以上: 对应敌人
 *  
 *        ※如果想要将效果限定为地图/战斗，
 *        　但不想将效果附加到任何角色，请输入screen。
 * 
 ...
 *  oldfilm //老电影
 *  pixelate //像素化
 *  reflection-m //波纹镜像-不可透
 *  reflection-w //波纹镜像-可透
 *  shockwave //冲击波
 *  tiltshift //倾斜位移
 *  zoomblur //缩放模糊
 *
 ...
 *        
 * 
 *   moveFilterQ [id] [该效果的参数 ...] [时间]
 *    移动和调整屏幕效果的参数。
 *     与moveFilter的不同之处在于，
 *     moveFilter取消之前的移动，按新的移动路线移动。
 *     如果连续执行的话，会一条一条地执行移动路线。
 *     （移动命令列）
 * 
 ...
 * 
 *   eraseFilterAfterMove [id]
 *    执行所有当前指定的移动路线后，自动删除画面效果。
 * 
 *    例: eraseFilterAfterMove 1
 * 
 * 
 *   setFilterSpeed [id] [变化速度]
 *    指定动画效果的变化速度。
 *     主要指godray(辐射), shockwave(波动), oldfilm(旧影像)
 *     默认: godray, shockwave : 0.01
 *     　　　　　  oldfilm           : 1
 *     　　　　　  crt               : 0.25
 *     　　　　　  reflection        : 0.1
 * 
 ...
 * 
 * 例如，想在setFilter中调整bulgePinch的参数时，
